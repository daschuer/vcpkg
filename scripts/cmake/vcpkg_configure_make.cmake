...
function(vcpkg_configure_make)
    ...
    if(NOT COMMAND _vcpkg_execute_in_download_mode)
        # Always save config.log to CURRENT_BUILDTREES_DIR, even on failure.
        function(vcpkg_execute_in_download_mode)
            _vcpkg_execute_in_download_mode(${ARGV})
            set(output_variables "RESULT_VARIABLE;RESULTS_VARIABLE;OUTPUT_VARIABLE;ERROR_VARIABLE")
            cmake_parse_arguments(PARSE_ARGV 0 arg "" "WORKING_DIRECTORY;${output_variables}" "")
            if(arg_WORKING_DIRECTORY AND EXISTS "${arg_WORKING_DIRECTORY}/config.log")
                get_filename_component(suffix "${arg_WORKING_DIRECTORY}" NAME)
                file(RENAME "${arg_WORKING_DIRECTORY}/config.log" "${CURRENT_BUILDTREES_DIR}/config.log-${suffix}.log")
            endif()
            foreach(var IN LISTS output_variables)
                z_vcpkg_forward_output_variable("arg_${var}" "${arg_${var}}")
            endforeach()
        endfunction()
    endif()
